...
class _RecipePageState extends State<RecipePage> {
  ...
  List<Map<String, dynamic>> _extractRecipes(String response) {
    ...
    for (var block in recipeBlocks) {
      ...
      final instructionSection =
          instructionSectionMatch?.group(1)?.trim() ?? '';
  ...
  void _addRecipeToFavorites(Map<String, dynamic> recipe) async {
    ...
    await dbHelper.insertFavoriteRecipe(
      ...
      List<String>.from(recipe['ingredients'] ??
          []), // Ensure this is passed as a List<String>
          ...
          children: isLoading && title == "New Recipes"
              ? [
                  Container(
                    ...
                    child: const Center(
                        child:
                            CircularProgressIndicator()), // Localized spinner
                  )
                ]
              : recipes.map((recipe) {
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        return GestureDetector(
                          onTap: () async {
                            final fridgeItems =
                                await fetchUserItemsWithExpiry(widget.userId);
                            final ingredientNames = fridgeItems
                                .map((item) => item['name'] ?? '')
                                .toList();
                              ...
                              MaterialPageRoute(
                                builder: (context) => ViewRecipePage(
                                  ...
                                  availableIngredients:
                                      ingredientNames, // Pass as List<String>
